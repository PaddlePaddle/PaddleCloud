...
class UserNotebook():
    ...
    def __create_deployment(self, username, namespace):
        v1beta1api = kubernetes.client.ExtensionsV1beta1Api(
            api_client=get_user_api_client(username))
        ...
        if not self.__find_item(dep_list, "cloud-notebook-deployment"):
            ...
            logging.info("command: %s, userid: %s", self.dep_body["spec"][
                "template"]["spec"]["containers"][0]["command"][2],
                         self.get_notebook_id(username))
            ...
            resp = v1beta1api.create_namespaced_deployment(
                namespace, body=dep_body, pretty=True)
    ...
    def __create_service(self, username, namespace):
        ...
        if not self.__find_item(service_list, "cloud-notebook-service"):
            resp = v1api.create_namespaced_service(
                namespace, body=self.service_body)
            self.__wait_api_response(resp)
    ...
    def __create_ingress(self, username, namespace):
        ...
        if not self.__find_item(ing_list, "cloud-notebook-ingress"):
            ...
            ing_body["spec"]["rules"][0]["http"]["paths"][0][
                "path"] = "/notebook/" + self.get_notebook_id(username)
            resp = v1beta1api.create_namespaced_ingress(
                namespace, body=ing_body)
            self.__wait_api_response(resp)
    ...
    def stop_all(self, username, namespace):
        ...
        v1api = kubernetes.client.CoreV1Api(
            api_client=get_user_api_client(username))
        v1beta1api.delete_namespaced_deployment("cloud-notebook-deployment",
                                                namespace)
        v1beta1api.delete_namespaced_ingress("cloud-notebook-ingress",
                                             namespace)
